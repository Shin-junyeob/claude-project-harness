...
> 로컬 프로젝트 폴더명은 한글을 써도 되지만, **GitHub 저장소 이름은 ASCII 만 허용**된다.
...
- `new-project.sh` 가 `bash team_repo_map.sh "<프로젝트명>"` 으로 호출해 repo 이름을 얻는다.
...
- **매핑에 없는 팀**: 영문 repo 명을 2번째 인자로 직접 지정한다.
  ...
  ```
- 매핑도 없고 인자도 없는데 폴더명이 **비ASCII** 면 **GitHub 연동만 건너뛴다**(로컬 프로젝트는 정상 생성). `"-"` 같은 잘못된 repo 가 만들어지지 않는다.
